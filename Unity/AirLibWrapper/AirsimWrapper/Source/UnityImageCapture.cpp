...
namespace AirSimUnity
{
	...
	void UnityImageCapture::getImages(const std::vector<msr::airlib::ImageCaptureBase::ImageRequest>& requests,
		std::vector<msr::airlib::ImageCaptureBase::ImageResponse>& responses, const std::string& vehicle_name) const
	{
		if (requests.size() > 0)
		{
			for (int i = 0; i < requests.size(); i++)
			{
				...
				AirSimImageResponse response = GetSimImages(request, vehicle_name.c_str());  //Into Unity
